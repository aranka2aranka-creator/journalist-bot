...
# مجموعه‌ی چت‌هایی که الان وسط یک گفتگوی مرحله‌ای هستند. تا وقتی چت‌آیدی‌ای
# اینجاست، on_message سراسری نباید به پیام‌های آن دخالت کند؛ چون آن پیام‌ها
# همزمان توسط bot.wait_for داخل ask_and_wait_text هم دریافت می‌شوند و اگر
# on_message هم واکنش نشان بدهد، گفتگو دوباره از اول (پیام «اول فایل را
# بفرستید») به نظر می‌رسد.
...
async def on_message(message: "bale.Message"):
    ...
    # اگر این چت وسط یک گفتگوی مرحله‌ای است، این پیام قرار است توسط
    # ask_and_wait_text (از طریق wait_for) پردازش شود؛ اینجا کاری نکن.
